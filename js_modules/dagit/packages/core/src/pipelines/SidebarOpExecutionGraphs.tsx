...
export const SidebarOpExecutionGraphs: React.FC<{
  ...
  const stats =
    result.data?.pipelineOrError.__typename === 'Pipeline'
      ? result.data.pipelineOrError.solidHandle?.solid.stepStats
      : undefined;
      ...
      <SidebarSection title={'Execution Status'}>
        <Box padding={{left: 24, right: 16, vertical: 12}}>
          <Box flex={{gap: 16}} style={{fontSize: '0.8rem'}}>
            <div style={{flex: 1}}>{`Last ${Math.min((stats || []).length, 10)} Runs`}</div>
            <Box style={{overflowX: 'auto'}} flex={{gap: 2}}>
              {(stats || [])
                .slice(0, 10)
                .reverse()
                .map(({runId, status, startTime}) => (
                  <Tooltip
                    key={runId}
                  ...
                  >
                    ...
                    >
                      <StepStatusDot
                        ...
                        onMouseOut={() => setHighlightedStartTime(null)}
                        style={{
                          border: `2px solid ${
